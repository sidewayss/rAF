...
// also standalone. Copyright (C) 2023 Sideways S, www.sidewayss.com
////////////////////////////////////////////////////////////////////////////////
    ...
    zvnet(isN, elms, v, m, f = undefined) {
        ...
        for (i = 0; i < x.length; i++) {
            if ((isN && x.vals[i][0]) || (!isN && x[i])) {
                ...
                for (i = 0; i < l; i++)
                    pv += vals[i] + seps[i];
                this.constructor.set(this, elms[i], pv);
            }                        // if no existing value, no way to plug it
        }
    }
    /////////////////////////////////// public static methods //////////////////
    static set(prop, elm, v) {       // Prop.set() handles CSS versus SVG
        if (prop.izSVG(elm)) //!!need a way to force use of .style, maybe elm.useStyle?
            elm.setAttribute(prop.name, v);
        else
            elm.style[prop.name] = v;
    }
...
/** @unrestricted */ class Easer {   // Subject of: "The easer eases the easee."
                ...
                b   = (c ? Prop.toBools(Prop.zap(ee.mask, f, a, p), c) : false))
    {
        ...
        o    = tc.zobj(max, min, a, f, p, c, b, ee.prop.plug(func), ee.isEnd);
        dims = Array.from(o, (v) => v.dims);
        d    = Math.max(...dims);
        if (!c && o.length == 3 && !Is.def(a) && o[0].value == Infinity
                                && !Is.def(p) && o[1].value ==-Infinity) {
            this.type = null;     ////\ apply raw ease().value to property
        ...
        if (!c) {                 ////\ single value with factor/addend/plug
            ...
            this.separator = ee.prop.separator(ee.func);
        }
        Object.seal(this);
    } ///////////////////////////////// end constructor() //////////////////////
    ...
    static zex(elms, ee, v = ee.prop.get(elms), b = false) {
        return b ? ee.prop.za(v, ee.func, ee.units)
                 : v.map(s => ee.prop.zparse(s, ee.func));
    ...
    apply(e) {                       // e = easeMe() return value
        ...
        if (ee.easy.byElm) {
            if (Is.def(this.turn)) {
                ...
                if (Is.A(tv))
                    tv = tv[this.idx];
                Prop.set(prop, this.elms[this.idx], tv);
                ee.turn(this);
            }
            Prop.set(prop, this.elms[this.idx], isAv ? v[this.idx] : v);
        }
        else if (isAv)
            this.elms.forEach((elm, i) => {Prop.set(prop, elm, v[i]);});
        else
            this.elms.forEach((elm)    => {Prop.set(prop, elm, v);});
    ...
    compute(ev) {
        ...
        case true:
            return Array.from(
                       {length:this.elms.length},
                       (_, i) => this.vfps(this.vnu(this.computer(ev, i), false, i), i)
                   );
        case 1:
            return this.vfps(this.varr(this.count, ev, this.plug)
                                 .join(this.separator));
        case 2:
            return Array.from(
                       {length:this.elms.length},
                       (_, i) => this.vfps(this.varr(this.count, ev, this.plug[i], i)
    ...
    vfps(v, i = 0) {                 // v   = value
        if (this.func)               // fps = func, prefix, suffix
            v = this.func.apply(v);
        if (this.easee.prop.isTransform)
...
class Geaser extends Easer {         // Easer for CSS gradients and for the SVG
    constructor(ee, elms) {          // <path d> and <poly* points> attributes.
        ...
        x  = ee.prop.getu(elms);     // x = parsed existing values object
      ...
      //b.bools = Array.from({length:c }, (_, i) => (nb + i) % 2);
        ...
        o = this.constructor.zobj(ee.max, ee.min, ee.addend, ee.factor,
                                  false,  false,  b, ee.prop.plug(), ee.isEnd);
...
class Teaser {                       // Easer for transforms, wraps 1+ Easers in
    constructor(ee, elms) {          // this.easers by Func instance.
        ...
        c   = Array.from({length:fc}, (_, i) => ee.prop.count(func[i]));
        ...
        for (i = 0; i < fc; i++) {  //\ compute the bools and create the Easers
            ...
            if (c[i]) {              // compute b: the bools object for fi
                m = Prop.svgRot(ee.prop.svg, fi, m, l, f, a, p);
                b = Prop.toBools(m, c[i]);
            }
            u = ee.prop.unitz(fi);   // one easer per func
            if (ee.units) {
    ...
    apply(e) { //////////////////////// apply() maintains func order by element
        ...
        if (ee.easy.byElm) {         // easy.byElm is loop by element
            if (Is.def(this.turn)) { // at the turning point, set values on two
                ...
                this.applyOne(ee.prop, tv, this.idx);
                ee.turn(this);
            }
            this.applyOne(ee.prop, v, this.idx);
        }
        else {
            for (let i = 0; i < this.elms.length; i++)
                this.applyOne(ee.prop, v, i);
        }
        if (ee.peri)                //\ run the callback
            ee.peri(this, e);
    }
    applyOne(prop, v, i) { ////////// applyOne() helps apply()
        let j, s, vf;
        for (s = "", j = 0; j < this.funcs.length; j++) {
            vf = v.get(this.felms ? this.felms[i][j] : this.funcs[j]);
            s += (Is.A(vf)  ? vf[i] : vf) + E.sp;
        }
        Prop.set(prop, this.elms[i], s.trimEnd());
    }
} ///////////////////////////////////// end class Teaser |||||||||||||||||||||||
///////////////////////////////////////|||||||||||||||||||||||||||||||||||||||||
class Easee {                        // Object of: "The easer eases the easee."
    constructor(ez, o) {
        this.set = o.set;    this.prop = o.prop;    this.easy = ez;
        ...
        this.func  = o.func  || o.prop.func;
        this.units = o.units || o.prop.unitz(this.func);
        this.byElm = o.byElm || o.byElement;
        this.is1m  = o.is1m  || o.oneMinus || o.complement;
        this.isEnd = Number(o.isEnd || Is.def(o.end));
        this.count = o.prop.count(o.func);
        this.isGDP = o.prop.isDPoints || (o.func && o.func.isGradient)
    ...
    static elmz(ee, elms) {          // kludgy to call setter from constructor
        ...
        ee.easer = !elms ? elms : (ee.prop.isTransform ? new Teaser(ee, elms)
...
class Easy { /*eslint-disable-line*/ // Outermost in the Easy instance hierarchy
    ...
    easeMe(now) {                    //\ easeMe() calculates and applies values
        ...
        let e = this.func(now);      // runs ease() or trip() to get e.value
        if (e.status !== E.pausing && !this.noop)
            this.targets.forEach((t) => { t.easer.apply(e); });
        if (this.peri)
             ...
             is2ndLeg)
    {
        ...
        if (!is2ndLeg)               // else this.now updated in trip()
            this.now = now;          // for pausing or reusing this Easy
        e.value = v;
        this.e  = e;
        return e;
