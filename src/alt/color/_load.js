...
function load(_, hasVisited) {
    ...
    elmsArray.push(...document.getElementsByTagName("color-swatch"));
...
//!!    start.forEach((_, i) => start[i] *= 255);
    ...
    ez2.newTarget({cjs, start, end, prop:P.color, elm:g.end.swatch});
    ...
    elm = [elms.controls, g.start.swatch, g.end.swatch];
...
function resizeWindow(_, isCollapsed) {
    ...
    for (id of START_END) {                 // position:absolute, their size and
        css = g[id].picker.style;           // position match their swatch, the
        elm = g[id].swatch.shadowRoot.getElementById("swatch");
        obj = elm.getBoundingClientRect();  // swatch is always visible and the
        css.width  = obj.width  + U.px;     // <input> has opacity:0, except on
        css.height = obj.height + U.px;     // :hover and :active.
